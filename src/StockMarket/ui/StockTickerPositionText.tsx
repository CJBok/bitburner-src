...
import { formatInt, formatPercent } from "../../ui/numeralFormat";
...
function LongPosition(props: IProps): React.ReactElement {
  ...
  return (
    <>
      ...
      <Typography>Shares: {formatInt(stock.playerShares)}</Typography>
      <Typography>
  ...
  if (Player.bitNodeN === 8 || Player.sourceFileLvl(8) >= 2) {
    return (
      <>
        ...
        <Typography>Shares: {formatInt(stock.playerShortShares)}</Typography>
        <Typography>
...
export function StockTickerPositionText(props: IProps): React.ReactElement {
  ...
  return (
    <>
      <Typography>Max Shares: {formatInt(stock.maxShares)}</Typography>
      <Typography>

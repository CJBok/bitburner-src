...
export function StockTicker(props: IProps): React.ReactElement {
  ...
  function getBuyTransactionCostContent(): JSX.Element | null {
    ...
    return (
      <>
        Purchasing {formatInt(qty)} shares ({position === PositionTypes.Long ? "Long" : "Short"}
  ...
  function getSellTransactionCostContent(): JSX.Element | null {
    ...
    return (
      <>
        Selling {formatInt(qty)} shares ({position === PositionTypes.Long ? "Long" : "Short"}) will result in a gain of{" "}
        <Money money={cost} />.
      </>
    );
  }

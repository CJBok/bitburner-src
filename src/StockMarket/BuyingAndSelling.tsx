...
import { formatInt, formatMoney } from "../ui/numeralFormat";
...
export function buyStock(
  ...
  if (shares + stock.playerShares + stock.playerShortShares > stock.maxShares) {
    if (ctx) {
      ...
      dialogBoxCreate(
        `You cannot purchase this many shares. ${stock.symbol} has a maximum of ${formatInt(stock.maxShares)} shares.`,
      );
    }
  ...
  if (ctx) {
    const resultTxt = `Bought ${formatInt(shares)} shares of ${stock.symbol} for ${formatMoney(
      totalPrice,
    ...
    dialogBoxCreate(
      <>
        Bought {formatInt(shares)} shares of {stock.symbol} for <Money money={totalPrice} />. Paid{" "}
...
export function sellStock(
  ...
  if (ctx) {
    const resultTxt =
      `Sold ${formatInt(shares)} shares of ${stock.symbol}. ` +
      `After commissions, you gained a total of ${formatMoney(gains)}.`;
    helpers.log(ctx, () => resultTxt);
  } else if (opts.suppressDialog !== true) {
    dialogBoxCreate(
      <>
        Sold {formatInt(shares)} shares of {stock.symbol}. After commissions, you gained a total of{" "}
        <Money money={gains} />.
      </>,
    );
  }
...
export function shortStock(
  ...
  if (ctx) {
    const resultTxt =
      `Bought a short position of ${formatInt(shares)} shares of ${stock.symbol} ` +
    ...
    dialogBoxCreate(
      <>
        Bought a short position of {formatInt(shares)} shares of {stock.symbol} for <Money money={totalPrice} />. Paid{" "}
        <Money money={CONSTANTS.StockMarketCommission} /> in commission fees.
      </>,
    );
  }
...
export function sellShort(
  ...
  if (ctx) {
    const resultTxt =
      `Sold your short position of ${formatInt(shares)} shares of ${stock.symbol}. ` +
      `After commissions, you gained a total of ${formatMoney(totalGain)}`;
    helpers.log(ctx, () => resultTxt);
  } else if (!opts.suppressDialog) {
    dialogBoxCreate(
      <>
        Sold your short position of {formatInt(shares)} shares of {stock.symbol}. After commissions, you gained a total
        of <Money money={totalGain} />
      </>,
    );
  }

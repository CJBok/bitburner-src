    ...
    <Container disableGutters maxWidth="md" sx={{ mx: 0 }}>
      ...
      <Box component={Paper} sx={{ p: 1, mb: 1 }}>
        ...
        <Typography>
          ...
          <b>Power:</b> {nFormat(AllGangs[gang.facName].power, 3)} <br />
...
function formatTerritory(n: number): string {
  ...
  const precision = 3;
  if (v <= 0) {
    return nFormat(0, precision);
  } else if (v >= 100) {
    return nFormat(100, precision);
  } else {
    return nFormat(v, precision);
  }
}

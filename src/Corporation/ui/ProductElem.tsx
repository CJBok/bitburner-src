...
export function ProductElem(props: IProductProps): React.ReactElement {
  ...
  if (product.sllman[city][0]) {
    if (isString(product.sllman[city][1])) {
      sellButtonText = (
        <>
          Sell ({nFormat(product.data[city][2])}/{product.sllman[city][1]})
        </>
      );
    } else {
      sellButtonText = (
        <>
          Sell ({nFormat(product.data[city][2])}/{nFormat(product.sllman[city][1])})
        </>
      );
    }
  } else {
  ...
  if (product.prdman[city][0]) {
    limitProductionButtonText += " (" + nFormat(product.prdman[city][1]) + ")";
  }
    ...
    <Paper>
      {!product.fin ? (
        <>
          ...
          <br />
          <Typography>{`${formatPercent(product.prog)} complete`}</Typography>
        ...
        <>
          <Box display="flex">
            <Tooltip
              title={
                <Typography>
                  Prod: {nFormat(product.data[city][1])}/s
                  <br />
                  Sell: {nFormat(product.data[city][2])} /s
                </Typography>
              }
            >
              <Typography>
                {product.name}: {nFormat(product.data[city][0])} ({nFormat(totalGain)}
                /s)
              </Typography>
            </Tooltip>
          </Box>
          <Box display="flex">
            <Tooltip
              title={
                <Typography>
                  Quality: {nFormat(product.qlt)} <br />
                  Performance: {nFormat(product.per)} <br />
                  Durability: {nFormat(product.dur)} <br />
                  Reliability: {nFormat(product.rel)} <br />
                  Aesthetics: {nFormat(product.aes)} <br />
                  Features: {nFormat(product.fea)}
                  {corp.unlockUpgrades[2] === 1 && <br />}
                  {corp.unlockUpgrades[2] === 1 && "Demand: " + nFormat(product.dmd)}
                  {corp.unlockUpgrades[3] === 1 && <br />}
                  {corp.unlockUpgrades[3] === 1 && "Competition: " + nFormat(product.cmp)}
                </Typography>
              }
            >
              <Typography>Rating: {nFormat(product.rat)}</Typography>
            </Tooltip>
          </Box>
          <Box display="flex">

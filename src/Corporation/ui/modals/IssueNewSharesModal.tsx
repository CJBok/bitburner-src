...
function EffectText(props: IEffectTextProps): React.ReactElement {
  ...
  return (
    <Typography>{`Issue ${nFormat(newShares)} new shares for ${formatMoney(newShares * newSharePrice)}?`}</Typography>
  );
}
...
export function IssueNewSharesModal(props: IProps): React.ReactElement {
  ...
  function issueNewShares(): void {
    ...
    let dialogContents =
      `Issued ${nFormat(newShares)} new shares` + ` and raised ${formatMoney(profit)}.` + (privateShares > 0)
        ? "\n" + nFormat(privateShares) + "of these shares were bought by private investors."
        : "";
  ...
  return (
    <Modal open={props.open} onClose={props.onClose}>
      <Typography>
        ...
        <br />
        <br />
        &nbsp;* You can issue at most {nFormat(maxNewShares)} new shares
        <br />

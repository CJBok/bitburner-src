...
export function Overview({ rerender }: IProps): React.ReactElement {
  ...
  function appendMult(name: string, value: number): void {
    if (value === 1) return;
    multRows.push([name, nFormat(value)]);
  }
    ...
    <>
      <StatsTable
        rows={[
          ...
          ["Owned Stock Shares:", nFormat(corp.numShares)],
      ...
      <Box display="flex">
        <Tooltip
          title={
            <StatsTable
              rows={[
                ["Outstanding Shares:", nFormat(corp.issuedShares)],
                ["Private Shares:", nFormat(corp.totalShares - corp.issuedShares - corp.numShares)],
              ]}
            />
          }
        >
          <Typography>Total Stock Shares: {nFormat(corp.totalShares)}</Typography>
        </Tooltip>
      </Box>
      <br />
  ...
  return (
    <StatsTable
      rows={[
        ...
        ["Dividend Percentage:", formatPercent(corp.dividendRate)],
